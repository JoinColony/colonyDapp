...
function* userTokensUpdate(
  ...
  try {
    ...
    const currentTokens = yield* executeQuery(queryContext, getUserTokens);
    const currentTokenAddresses = currentTokens.map(({ address }) => address);

    yield* executeCommand(commandContext, updateTokens, {
      tokens,
      currentTokens: currentTokenAddresses,
    });
