...
class AdditionalOmniPicker extends Component<Props> {
  ...
  handleOutsideClick = (evt: MouseEvent) => {
    const { inputRef, close, triggerElement } = this.props;
    ...
    if (
      ...
      (evt.target instanceof Node &&
        inputRef &&
        inputRef.contains(evt.target)) ||
      (evt.target instanceof Node &&
        triggerElement &&
        triggerElement.contains(evt.target))
    )
      return;
