...
function* taskSetBounty({
  ...
  meta,
}: Action<typeof ACTIONS.TASK_SET_BOUNTY>): Saga<void> {
  try {
    const context = yield* getStoreContext(colonyENSName, taskStoreAddress);
    yield* executeCommand(context, setTaskBounty, { token, amount });
    yield put({
      type: ACTIONS.TASK_SET_BOUNTY_SUCCESS,
      payload: {
        taskId,
        amount,
      },
    ...
    yield putError(ACTIONS.TASK_SET_BOUNTY_ERROR, error, meta);
  }
}
...
export default function* tasksSagas(): any {
  ...
  yield takeEvery(ACTIONS.TASK_SET_BOUNTY, taskSetBounty);
