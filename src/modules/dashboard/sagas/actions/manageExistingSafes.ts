...
function* manageExistingSafesAction({
  ...
  meta,
}: Action<ActionTypes.COLONY_ACTION_MANAGE_EXISTING_SAFES>) {
  let txChannel;
  try {
    ...
    yield put<AllActions>({
      type: ActionTypes.COLONY_ACTION_MANAGE_EXISTING_SAFES_SUCCESS,
      meta,
    });
    ...
    return yield putError(
      ActionTypes.COLONY_ACTION_MANAGE_EXISTING_SAFES_ERROR,
      error,
      meta,
    );
  } finally {
...
export default function* addExistingSafeSaga() {
  yield takeEvery(
    ActionTypes.COLONY_ACTION_MANAGE_EXISTING_SAFES,
    manageExistingSafesAction,
  );
}
