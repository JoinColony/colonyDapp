  ...
  Query: {
    async colonyAction(_, { transactionHash, colonyAddress }) {
      ...
      if (transactionReceipt) {
        ...
        // @NOTE: We filter out duplicated events created by making Safe multi-transactions.
        const uniqueReverseSortedEvents = uniqWith(
          reverseSortedEvents,
          (valueA, valueB) =>
        ...
        let actionType;
        const motionCreatedEvent = uniqueReverseSortedEvents.find(
        ...
        if (motionCreatedEvent) {
          ...
          actionType = getActionType(uniqueReverseSortedEvents);
        }
        const actionValues = await getActionValues(
          uniqueReverseSortedEvents,
          colonyClient as ColonyClient,
          votingClient as ExtensionClient,
          oneTxPaymentClient as ExtensionClient,
          apolloClient,
        ...
        let events = uniqueReverseSortedEvents;
        /*
