...
export const getUserColonyTransactions: UserColonyTransactionsQuery<void> = ({
  ...
  async execute() {
    ...
    const toTransferLogs = await colonyClient.token.getLogs({
      ...baseLog,
      // [eventNames, from, to]
      topics: [[], filterFormattedAddress],
    });

    const fromTransferLogs = await colonyClient.token.getLogs({
      ...baseLog,
      // [eventNames, from, to]
    ...
    const transferEvents = await colonyClient.token.parseLogs(transferLogs);
