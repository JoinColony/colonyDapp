...
import { getNetworkMethod } from './utils';
...
function* createColonySagaNew(
  ...
  action: $PropertyType<ActionsType, 'COLONY_CREATE_NEW'>,
): Saga<void> {
...
function* createColonySaga({
  payload: params,
  meta,
}: $PropertyType<ActionsType, 'COLONY_CREATE'>): Saga<void> {
  yield put(
    createColony({
      meta,
      params,
...
function* createColonyLabelSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'COLONY_CREATE_LABEL'>): Saga<void> {
...
function* validateColonyDomain({
  payload: { ensName },
  meta,
}: $PropertyType<ActionsType, 'COLONY_DOMAIN_VALIDATE'>): Saga<void> {
  yield delay(300);
  ...
  yield put({ type: ACTIONS.COLONY_DOMAIN_VALIDATE_SUCCESS, meta });
}
...
}: $PropertyType<ActionsType, 'COLONY_CREATE_LABEL_SUCCESS'>): Saga<void> {
...
}: $PropertyType<ActionsType, 'COLONY_PROFILE_UPDATE'>): Saga<void> {
  try {
    /*
...
function* fetchColonySaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'COLONY_FETCH'>): Saga<void> {
  try {
...
function* fetchColonyENSName({
  ...
  meta,
}: $PropertyType<ActionsType, 'COLONY_ENS_NAME_FETCH'>): Saga<void> {
  try {
...
function* uploadColonyAvatar({
  ...
  meta,
  payload: data,
}: $PropertyType<ActionsType, 'COLONY_AVATAR_UPLOAD'>): Saga<void> {
  try {
...
}: $PropertyType<ActionsType, 'COLONY_AVATAR_FETCH'>): Saga<void> {
  try {
    /*
...
}: $PropertyType<ActionsType, 'COLONY_AVATAR_REMOVE'>): Saga<void> {
  try {
    /*
     * Get the colony store
     */
