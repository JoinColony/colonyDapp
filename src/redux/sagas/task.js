...
import { createBatchTxRunner } from './transactions';
...
import { allColonyENSNames } from '~redux/selectors';
...
function* taskCreateSaga(
  action: $PropertyType<ActionsType, 'TASK_CREATE'>,
): Saga<void> {
  const {
    meta,
    payload: {
...
function* taskModifyWorkerPayoutSaga(
  action: $PropertyType<ActionsType, 'TASK_MODIFY_WORKER_PAYOUT'>,
): Saga<void> {
  const {
    meta,
...
function* taskFetchSaga({
  ...
  meta,
  payload,
}: $PropertyType<ActionsType, 'TASK_FETCH'>): Saga<void> {
  try {
...
function* taskUpdateSaga({
  ...
  meta,
  payload,
}: $PropertyType<ActionsType, 'TASK_UPDATE'>): Saga<void> {
  try {
    yield call(ensureColonyIsInState, colonyENSName);
...
function* taskRemoveSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'TASK_REMOVE'>): Saga<void> {
  try {
    yield call(ensureColonyIsInState, colonyENSName);
     ...
     * Dispatch the success action.
     */
    yield put({ type: ACTIONS.TASK_REMOVE_SUCCESS, meta });
  } catch (error) {
...
function* taskSetSkillSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'TASK_SET_SKILL'>): Saga<void> {
  yield put(
    setTaskSkillTx({
      identifier: colonyENSName,
...
function* taskSetDueDateSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'TASK_SET_DATE'>): Saga<void> {
  yield put(
    setTaskDueDateTx({
      identifier: colonyENSName,
...
function* taskWorkerEndSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'TASK_WORKER_END'>): Saga<void> {
...
function* completeTaskSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'TASK_MANAGER_END'>): Saga<void> {
  try {
...
function* taskWorkerRateManagerSaga({
  payload: { colonyENSName, taskId, rating },
  meta,
}: $PropertyType<ActionsType, 'TASK_WORKER_RATE_MANAGER'>): Saga<void> {
  try {
    // generate secret
    const secret = yield call(
      generateRatingSaltAndSecret,
...
function* taskManagerRateWorkerSaga({
  payload: { colonyENSName, taskId, rating },
  meta,
}: $PropertyType<ActionsType, 'TASK_MANAGER_RATE_WORKER'>): Saga<void> {
  try {
    // generate secret
    const secret = yield call(
      generateRatingSaltAndSecret,
...
function* taskWorkerClaimRewardSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'TASK_WORKER_CLAIM_REWARD'>): Saga<void> {
  yield all(
...
function* taskFinalizeSaga({
  ...
  meta,
}: $PropertyType<ActionsType, 'TASK_FINALIZE'>): Saga<void> {
  yield put(
